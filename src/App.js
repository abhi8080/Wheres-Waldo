...
document.addEventListener("click", (event) => {
  if (event.target.id == "photo")
    photoIsClicked(event)
  else if(event.target.id == "view-characters")
       openCharactersBox()
    ...
    coordinates.then((coordinate) =>  {
      if(isXcoordinateInRange(coordinate.xValue, xCoordinateClicked) && isYcoordinateInRange(coordinate.yValue, yCoordinateClicked)) {
...
function isXcoordinateInRange(characterXcoordinate, xOnScreen) {
  if(xOnScreen > (characterXcoordinate - 10) && xOnScreen <  (characterXcoordinate + 10))
    return true;
  else
    return false;
}
...
function isYcoordinateInRange(characterYcoordinate, yOnScreen) {
  if(yOnScreen > (characterYcoordinate - 10) && yOnScreen <  (characterYcoordinate + 10))
    return true;
  else
    return false;
}
